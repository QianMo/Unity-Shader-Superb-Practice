//Draws a pattern with circles.
// Shader source :
// https://pastebin.com/a3dFXMhR
 
// Based on :
// https://thebookofshaders.com/10/

Shader "ShaderSuperb/Session19/Noises or Patterns/Noise/Truchet - Circles"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f_img i) : SV_Target
			{
				...
				fixed val = (step(length(tile), 0.6) - step(length(tile), 0.4))
				+ (step(length(tile - float2(1.0, 1.0)), 0.6) - step(length(tile - float2(1.0, 1.0)), 0.4));
