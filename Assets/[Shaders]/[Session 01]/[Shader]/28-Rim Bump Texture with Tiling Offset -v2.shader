...
Shader "ShaderSuperb/Session1/27-Rim Bump Texture with Tiling Offset"
{
    Properties
    {
    ...
    SubShader
    {
        Pass
        {
            ...
            float4 frag(v2f i):SV_Target
            {
                ...
                //计算出边缘自发光强度  
                float3 RimLighting = max(0, dot(i.worldSpaceNormalDir, WorldSpaceViewDir) *_RimColor.rgb * _LightColor0.rgb* pow(Rim , _RimPower) *_RimIntensity );  


                //最终像素颜色
                float3 FinalColor = MainTexColor * BumpIntensity + RimLighting;
